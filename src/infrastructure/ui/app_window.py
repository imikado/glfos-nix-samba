...
class MainWindow(Adw.ApplicationWindow):
        ...
        try:
            if not os.path.exists(requirements_domain.get_config_file_path()):
                missing_items.append(
                    ...
                    + requirements_domain.get_config_file_path()
                )
            else:
    ...
    def _show_config_warning(self, missing_items):
        ...
        body = (
            _("The following items are missing from your NixOS configuration:") + "\n\n"
        )
    ...
    def _show_fix_requirements_dialog(self):
        ...
        dialog.set_body(
            _(
                "The NixOS configuration is missing required settings (samba.nix and samba_setup.nix import). Enter your password to fix it automatically."
            )
        )
    ...
    def _on_close_request(self, _window):
        if self._remote_domain.need_to_save():
            ...
            dialog.set_body(
                _(
                    "You have modifications not saved. Are you sure you want to quit without saving?"
                )
            )
            dialog.add_response("cancel", _("Cancel"))
    ...
    def on_create_creds_file(self, _button):
        ...
        page = CreateCredsFilePage(
            credentials_file_domain, self.navigation_view, self.show_notification
        )
        self.navigation_view.push(page)
    ...
    def _on_save_password_response(self, dialog, response):
        ...
        try:
            ...
            rebuild_bash_path = system_api.write_rebuild_bash(
                tmp_samba_nix_path, password
            )
